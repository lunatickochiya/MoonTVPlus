      ...
      <div data-tv-focus-scope='active'>
        <section className='mx-auto max-w-6xl rounded-[42px] border border-white/10 bg-slate-950/70 p-10 shadow-2xl shadow-black/60'>
          <div
            className={
              localRemoteUrl
                ? 'grid items-center gap-10 lg:grid-cols-[minmax(0,1fr)_208px]'
                : undefined
            }
          >
            <div>
              ...
              <form onSubmit={submit} className='mt-10 flex gap-4'>
                ...
                <input
                  ...
                  className='h-20 min-w-0 flex-1 rounded-3xl border border-white/10 bg-white/10 px-8 text-3xl text-white outline-none placeholder:text-slate-500 focus:border-rose-500 tv-focusable'
                />
                <button
                  type='submit'
                  className='flex h-20 shrink-0 cursor-pointer items-center gap-3 rounded-3xl bg-rose-600 px-8 text-3xl font-black text-white outline-none transition hover:bg-rose-500 tv-focusable'
                >
            ...
            {localRemoteUrl && (
              <div className='flex flex-col items-center'>
                <div className='mb-3 text-center text-lg font-black text-indigo-200'>
                  手机输入与遥控
                </div>
                <TVLocalRemoteQRCode remoteUrl={localRemoteUrl} compact />
              </div>
            )}
          </div>
        </section>
        {(loading || searched || error) && (
          <section className='mx-auto mt-12 max-w-6xl'>
            ...
            {error ? (
              <div className='rounded-3xl border border-red-500/40 bg-red-950/40 p-8 text-2xl font-bold text-red-100'>
                {error}
              </div>
            ) : loading ? null : displayResults.length === 0 ? (
              <div className='rounded-3xl border border-white/10 bg-white/[0.06] p-8 text-2xl font-bold text-slate-300'>
                未找到相关结果
              </div>
            ) : (
              <div className='grid grid-cols-2 gap-5 lg:grid-cols-4'>
                {displayResults.map((item, index) => (
                  ...
                  >
                    ...
                    <div className='p-5'>
                      <div className='line-clamp-1 text-2xl font-black text-white'>
                        {item.title}
                      </div>
                      <div className='mt-2 line-clamp-1 text-lg font-bold text-slate-300'>
                        {item.isAggregate
                          ? `${item.sourceNames.length} 个播放源`
                          : item.sourceName}
                      </div>
            ...
            <div className='mt-6 grid grid-cols-2 gap-4 md:grid-cols-4'>
              {history.slice(0, 20).map((item) => (
                <button
                  ...
                  className='cursor-pointer rounded-3xl border border-white/10 bg-white/[0.06] px-6 py-5 text-2xl font-bold text-white outline-none transition hover:bg-white/12 tv-focusable'
                >
                  {item}
                </button>
              ))}
